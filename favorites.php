        ...
        <main>
            <section class="products">
                <?php
                    ...
                    $sel = mysqli_query($connect, "SELECT * FROM favorites INNER JOIN items ON favorites.itemid = items.itemid WHERE favorites.login = '".$_SESSION['login']."';"); 
                    while($data = mysqli_fetch_array($sel)){
                        echo '<div>';
                        echo '<img src = "'.$data['photo'].'"alt = "'.$data['name'].'">';
                        echo '<h3>'.$data['name'].'</h3>';
                        echo '<button onclick = "deleteFromFavorites('.$data['itemid'].')" class="add-to-favorites">🗑️</button>';
                        echo '</div>';
                    }
                ?>
            </section>
